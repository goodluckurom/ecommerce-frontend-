...
const AllShopRefundOrder = () => {
  ...
  const paginatedData = refundOrders.slice(startIndex, endIndex);
  ...
  return (
    <div className="all-orders w-full pt-1 mt-10">
      ...
      <div className="pagination">
        ...
        <button
          onClick={() =>
            setPageIndex((prevIndex) =>
              Math.min(
                prevIndex + 1,
                Math.ceil(refundOrders.length / pageSize) - 1
              )
            )
          }
          disabled={pageIndex === Math.ceil(refundOrders.length / pageSize) - 1}
          className={`pagination-btn ${
            pageIndex === Math.ceil(refundOrders.length / pageSize) - 1 &&
        ...
        Page
        <strong>
          {pageIndex + 1} of {Math.ceil(refundOrders.length / pageSize)}
        </strong>{" "}
      </div>
    </div>
  );
...
export default AllShopRefundOrder;
